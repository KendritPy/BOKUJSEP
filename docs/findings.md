...
- `0x0892EBA4-0x0892EBB1` currently contains `8D006A008D006A00950070009500`. Interpreted as little-endian 16-bit values this strongly resembles repeated 2D coordinates (for example 141,106,141,106,149,112,...), consistent with glyph/quad/layout geometry. This is a hypothesis, not yet proved.
...
- `probe-watch-glyph.bat` now watches the first non-code survivor (`0x0892EBA4`, 14 bytes) using real breakpoint broadcasts. The goal is to identify the function that builds line-dependent render/layout data and trace backward toward the text parser.
