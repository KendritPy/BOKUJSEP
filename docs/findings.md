...
- Classification of the 21 surviving stable-diff runs found 19 inside known functions and only two `DATA_OR_UNKNOWN` runs: `0x0892EBA4-0x0892EBB1` and `0x0892EDB0-0x0892EDB7`, both within the active `bnp` module.
- `0x0892EBA4-0x0892EBB1` was hypothesized to be glyph/layout geometry because one observation contained repeated small halfword coordinate-like values. A real change watch on that range later timed out on a normal textbox advance, and a later baseline at the same address differed substantially. It is not currently a reliable transition target.
- `0x0892EDB0-0x0892EDB7` contains repeated `B40E` halfwords and also has no known function covering it. Its semantic role is currently unknown.
...
- A more fundamental PPSSPP debugger pitfall was identified: `memory.read` defaults `replacements=true`. PPSSPP's own source documents that `replacements=false` is required when a debugger wants original guest code rather than PPSSPP replacement/emuhack instructions. Earlier full-RAM snapshots used the default and therefore allowed emulator-generated code substitutions to contaminate differential results. `PPSSPPDebugger.read()` now defaults to `replacements=false` and verifies exact response length. All full-RAM differential work before this fix must be treated as provisional and rerun.
- `dialogue_watch_event_probe.py` now also reads with `replacements=false` and writes a JSON report on timeout as well as on a successful hit.
